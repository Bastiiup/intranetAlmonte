...
### Causa:
El campo `año` estaba configurado como `required: true` en el schema de `cursos`. Cuando se hace un PUT para actualizar solo `versiones_materiales` sin incluir `año`:
...
El código del frontend (`AlmonteIntranet/src/app/api/crm/cursos/import-pdf/route.ts`) ya está preparado para esta solución:

1. ✅ **No incluye `año` si no existe o es inválido** (líneas 195-214)
2. ✅ **Solo incluye `año` si es un número válido** (validación con `isFinite()`)
3. ✅ **Logs de debugging** para identificar problemas
4. ✅ **Manejo de errores** robusto

**El frontend está listo y funcionará correctamente después del rebuild de Strapi.**
