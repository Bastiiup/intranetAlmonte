...
export default function LogsViewer() {
  ...
  const fetchLogs = async () => {
    try {
      ...
      const response = await fetch(`/api/crm/listas/debug-logs?${params.toString()}`)
  ...
  const exportLogs = () => {
    ...
    a.href = url
    a.download = `logs-listas-${new Date().toISOString().split('T')[0]}.txt`
        ...
        <Col xs={12}>
          <Card>
            <CardHeader className="d-flex justify-content-between align-items-center">
              <div>
                <h4 className="mb-0">📋 Logs de Procesamiento de Listas</h4>
                <small className="text-muted">Logs del sistema de procesamiento de PDFs con IA</small>
              </div>
                  ...
                  <div className="text-center text-muted py-5">
                    ...
                    <small>
                      {searchTerm || levelFilter !== 'all' 
                        ...
                        : 'Los logs aparecerán aquí cuando se procesen PDFs'}
                    </small>
                  </div>
                ) : (
