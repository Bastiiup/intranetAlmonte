...
const EditOpportunityModal = ({ show, onHide, opportunity, onSuccess }: EditOpportunityModalProps) => {
  ...
  // Cargar datos completos de la oportunidad cuando se abre el modal
  useEffect(() => {
    if (show && opportunity?.realId) {
      loadOpportunityData()
      loadData()
      setError(null)
    } else if (show && opportunity) {
      // Fallback: usar datos limitados de OpportunitiesType
      const amountMatch = opportunity.amount.match(/[\d,]+/)
      const monto = amountMatch ? amountMatch[0].replace(/,/g, '') : ''
      ...
      const etapa = etapaMap[opportunity.stage] || opportunity.stage

      setFormData({
        nombre: opportunity.productName || '',
        descripcion: '',
        monto: monto,
        moneda: opportunity.amount.includes('USD') ? 'USD' : opportunity.amount.includes('EUR') ? 'EUR' : 'CLP',
        etapa: etapa,
        estado: opportunity.status || 'open',
        prioridad: opportunity.priority || 'medium',
        fecha_cierre: opportunity.closeDate && opportunity.closeDate !== '-' ? new Date(opportunity.closeDate).toISOString().split('T')[0] : '',
        fuente: opportunity.source || 'Manual',
        contacto: '',
        producto: '',
        propietario: '',
      })
  ...
  const loadOpportunityData = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        let contactoId = ''
        if (data.contacto) {
          const contacto = data.contacto.data || data.contacto
          contactoId = contacto?.documentId || contacto?.id || ''
        }

        let productoId = ''
        if (data.producto) {
          const producto = data.producto.data || data.producto
          productoId = producto?.documentId || producto?.id || ''
        }

        let propietarioId = ''
        if (data.propietario) {
          const propietario = data.propietario.data || data.propietario
          propietarioId = propietario?.documentId || propietario?.id || ''
        }
        ...
        setFormData({
          ...
          etapa: data.etapa || 'Qualification',
          estado: data.estado || 'open',
          prioridad: data.prioridad || 'medium',
          fecha_cierre: data.fecha_cierre ? new Date(data.fecha_cierre).toISOString().split('T')[0] : '',
          fuente: data.fuente || 'Manual',
          contacto: String(contactoId),
          producto: String(productoId),
          propietario: String(propietarioId),
        })
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        const errorMessage = result.error || result.details?.errors?.[0]?.message || 'Error al actualizar oportunidad'
    ...
    <Modal show={show} onHide={onHide} size="lg" centered>
      <Form onSubmit={handleSubmit}>
        <ModalHeader closeButton>
          <ModalTitle>Editar Oportunidad</ModalTitle>
        </ModalHeader>
        <ModalBody>
          {error && (
