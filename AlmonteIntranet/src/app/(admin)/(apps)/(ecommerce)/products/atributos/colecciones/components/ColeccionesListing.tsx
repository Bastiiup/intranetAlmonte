...
// Cargar el formulario de agregar colección dinámicamente
const AddColeccionForm = dynamic(
  () => import('../components/AddColeccionForm').then((mod) => ({ default: mod.default })),
  { ssr: false }
)
...
type TabType = 'colecciones' | 'solicitudes' | 'agregar'
...
const ColeccionesListing = ({ colecciones, error }: ColeccionesListingProps = {}) => {
  ...
  const [activeTab, setActiveTab] = useState<TabType>('colecciones')
  ...
  // Renderizar el contenido del tab de colecciones
  const renderColeccionesContent = () => (
    <Card className="mb-4">
  ...
  return (
    <Row>
      <Col xs={12}>
        <TabContainer defaultActiveKey="colecciones" activeKey={activeTab} onSelect={(k) => setActiveTab(k as TabType)}>
          <Card className="mb-4">
            <CardHeader>
              <Nav variant="tabs" className="card-header-tabs nav-bordered">
                <NavItem>
                  <NavLink eventKey="colecciones" className="d-flex align-items-center gap-2">
                    <TbList size={18} />
            ...
            <CardBody>
              <TabContent>
                <TabPane eventKey="colecciones">
                  {renderColeccionesContent()}
                </TabPane>
                {canViewRequests && (
