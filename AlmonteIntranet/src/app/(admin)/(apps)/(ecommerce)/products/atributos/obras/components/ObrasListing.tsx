...
// Cargar el formulario de agregar obra dinámicamente
const AddObraForm = dynamic(
  () => import('../components/AddObraForm').then((mod) => ({ default: mod.default })),
  { ssr: false }
)
...
type TabType = 'obras' | 'solicitudes' | 'agregar'
...
const ObrasListing = ({ obras, error }: ObrasListingProps = {}) => {
  ...
  // Renderizar el contenido del tab de obras
  const renderObrasContent = () => (
    <Card className="mb-4">
  ...
  return (
    <Row>
      <Col xs={12}>
        <TabContainer defaultActiveKey="obras" activeKey={activeTab} onSelect={(k) => setActiveTab(k as TabType)}>
          <Card className="mb-4">
            <CardHeader>
              <Nav variant="tabs" className="card-header-tabs nav-bordered">
                <NavItem>
                  <NavLink eventKey="obras" className="d-flex align-items-center gap-2">
                    <TbList size={18} />
