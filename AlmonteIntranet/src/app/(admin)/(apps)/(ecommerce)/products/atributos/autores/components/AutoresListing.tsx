...
// Cargar el formulario de agregar autor dinámicamente
const AddAutorForm = dynamic(
  () => import('../components/AddAutorForm').then((mod) => ({ default: mod.default })),
  { ssr: false }
)
...
type TabType = 'autores' | 'solicitudes' | 'agregar'
...
const AutoresListing = ({ autores, error }: AutoresListingProps = {}) => {
  ...
  // Renderizar el contenido del tab de autores
  const renderAutoresContent = () => (
    <Card className="mb-4">
  ...
  return (
    <Row>
      <Col xs={12}>
        <TabContainer defaultActiveKey="autores" activeKey={activeTab} onSelect={(k) => setActiveTab(k as TabType)}>
          <Card className="mb-4">
            <CardHeader>
              <Nav variant="tabs" className="card-header-tabs nav-bordered">
                <NavItem>
                  <NavLink eventKey="autores" className="d-flex align-items-center gap-2">
                    <TbList size={18} />
            ...
            <CardBody>
              <TabContent>
                <TabPane eventKey="autores">
                  {renderAutoresContent()}
                </TabPane>
                {canViewRequests && (
