...
// Cargar el formulario de agregar sello dinámicamente
const AddSelloForm = dynamic(
  () => import('../components/AddSelloForm').then((mod) => ({ default: mod.default })),
  { ssr: false }
)
...
type TabType = 'sellos' | 'solicitudes' | 'agregar'
...
const SellosListing = ({ sellos, error }: SellosListingProps = {}) => {
  ...
  const [activeTab, setActiveTab] = useState<TabType>('sellos')
  ...
  // Renderizar el contenido del tab de sellos
  const renderSellosContent = () => (
    <Card className="mb-4">
  ...
  return (
    <Row>
      <Col xs={12}>
        <TabContainer defaultActiveKey="sellos" activeKey={activeTab} onSelect={(k) => setActiveTab(k as TabType)}>
          <Card className="mb-4">
            <CardHeader>
              <Nav variant="tabs" className="card-header-tabs nav-bordered">
                <NavItem>
                  <NavLink eventKey="sellos" className="d-flex align-items-center gap-2">
                    <TbList size={18} />
                    <span>Todos los Sellos</span>
                  </NavLink>
                </NavItem>
                {canViewRequests && (
                  <NavItem>
